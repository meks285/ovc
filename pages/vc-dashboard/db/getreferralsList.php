...
$sel = mysqli_query($conn,"select count(*) as allcount from referralform where vc_unique_id='$vc_unique_id'");
$records = mysqli_fetch_assoc($sel);
//$totalRecords = $records['allcount'];
$totalRecords = isset($records['allcount']) ? $records['allcount'] : 0;

## Total number of record with filtering
$sel = mysqli_query($conn,"select count(*) as allcount from referralform where vc_unique_id='$vc_unique_id' AND 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
if(isset($searchValue) && $searchValue != ''){
    ...
    `reg_date` 
  FROM
    `referralform` where vc_unique_id='$vc_unique_id' AND 1 ".$searchQuery;
}
else{
    $empQuery = "SELECT 
    `id`,
    ...
    `reg_date` 
  FROM
    `referralform` where vc_unique_id='$vc_unique_id' order by reg_date desc";
}

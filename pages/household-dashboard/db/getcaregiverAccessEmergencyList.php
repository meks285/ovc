...
$sel = mysqli_query($conn,"select count(*) as allcount from checklist_accesstoemergencyfunds where hh_unique_num='$householdUniqueId'");
$records = mysqli_fetch_assoc($sel);
//$totalRecords = $records['allcount'];
$totalRecords = isset($records['allcount']) ? $records['allcount'] : 0;

## Total number of record with filtering
$sel = mysqli_query($conn,"select count(*) as allcount from checklist_accesstoemergencyfunds where hh_unique_num='$householdUniqueId' AND 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
...
if(isset($searchValue) && $searchValue != ''){
    ...
    `reg_date` 
  FROM
    `checklist_accesstoemergencyfunds` where hh_unique_num='$householdUniqueId' AND 1 ".$searchQuery;
}
else{
    $empQuery = "SELECT 
    `id`,
    ...
    `reg_date` 
  FROM
    `checklist_accesstoemergencyfunds` where hh_unique_num='$householdUniqueId' order by reg_date asc";
}
